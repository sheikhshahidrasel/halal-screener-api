...
def get_asia_tickers():
    ...
    # 1. Fetch ALL Indian Stocks directly from Official NSE CSV
    try:
        print("Fetching official NSE (India) stock list...")
        nse_url = "https://archives.nseindia.com/content/equities/EQUITY_L.csv"
        res = session.get(nse_url, headers=headers, timeout=15)
        ...
        if res.status_code == 200:
            ...
            print("Successfully loaded Indian tickers from NSE.")
    ...
    for url, suffix, is_hk in wiki_sources:
        try:
            res_wiki = session.get(url, headers=headers, timeout=10)
    ...
    # 3. Systematic generation for highly active Asian markets
...
def check_shariah_compliance(ticker):
    try:
        ...
        # Fast exit for invalid/empty generated tickers
        info = stock.info
        ...
        # Silently fail for generated tickers that don't exist
        return None
...
def update_asia_stocks():
    ...
    print("Starting Asia data extraction...")
    for ticker in tickers:
        ...
        # Maintained original Anti-ban sleep timing
    ...
    print(f"\n--- SUCCESS ---")
    print(f"Data saved: {len(stock_data)} valid Asian stocks processed successfully into asia_data.json.")
