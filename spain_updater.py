...
def get_spain_tickers():
    ...
    # Fetch Major Spain Indices via Wikipedia Automatically (Bolsa de Madrid)
    wiki_sources = [
        ("https://en.wikipedia.org/wiki/IBEX_35", "MC")
    ]
    
    print("Fetching official Spain (Bolsa de Madrid) stock list automatically...")
    for url, suffix in wiki_sources:
        fetched = fetch_wiki_tickers(session, url, suffix)
        spain_tickers.update(fetched)
        print(f"Loaded {len(fetched)} tickers from index source.")
        time.sleep(2)

    final_tickers = list(spain_tickers)
    print(f"Successfully compiled a total of {len(final_tickers)} unique Spain tickers.")
    
    # Fallback in case Wikipedia structure changes or network fails
    if len(final_tickers) < 5:
         return ["IBE.MC", "SAN.MC", "ITX.MC", "BBVA.MC", "REP.MC"]
...
def update_spain_stocks():
    tickers = get_spain_tickers()
    stock_data = []
    count = 0

    print("Starting Spain data extraction...")
    ...
    # Saved entirely as a Spain specific JSON
    with open('spain_data.json', 'w') as f:
        json.dump(stock_data, f, indent=4, cls=NumpyEncoder)
        
    print(f"\n--- SUCCESS ---")
    print(f"Data saved: {len(stock_data)} valid Spain stocks processed successfully into spain_data.json.")
