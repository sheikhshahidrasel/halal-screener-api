...
def get_us_tickers():
    try:
        ...
        return ["AAPL", "MSFT", "GOOGL", "AMZN", "META", "TSLA", "ORCL", "JPM", "BAC"]
...
def extract_financial_value(dataframe, keys):
    ...
    for key in keys:
        if key in dataframe.index:
            ...
            # If multiple columns/series returned, take the first one (most recent)
...
def generate_scholarly_report(company_name, sector, industry, is_haram_business, missing_data, ratios, passes):
    ...
    if is_haram_business:
        # If business is haram, financials don't matter, but we show them anyway for transparency
...
def check_shariah_compliance(ticker):
    try:
        ...
        # Generate the Deep Premium Narrative Report
...
def update_us_stocks_part_2():
    tickers = get_us_tickers()
    
    # SLICING LOGIC: Take the SECOND HALF of the tickers
    mid_point = len(tickers) // 2
    tickers_part_2 = tickers[mid_point:]
    
    print(f"Processing US Part 2: {len(tickers_part_2)} tickers...")
    stock_data = []

    for ticker in tickers_part_2:
    ...
    print(f"\n--- SUCCESS ---")
    print(f"Successfully processed {len(stock_data)} US stocks and saved to us_data_2.json.")

if __name__ == "__main__":
    update_us_stocks_part_2()
