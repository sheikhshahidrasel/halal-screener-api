...
def get_uk_tickers():
    ...
    # Automatically Fetch UK Stocks (LSE) list
    try:
        ...
        for ticker in df[col].dropna().astype(str):
            ...
            if clean_ticker and clean_ticker.upper() != 'SYMBOL':
                if not clean_ticker.endswith(".L"):
                     uk_tickers.add(f"{clean_ticker}.L")
                else:
                     uk_tickers.add(clean_ticker)
        print(f"Successfully loaded {len(uk_tickers)} LSE (UK) tickers.")
    except Exception as e:
        print(f"Error fetching LSE tickers: {e}")

    final_tickers = list(uk_tickers)
    
    # Fallback in case of network issues
    if len(final_tickers) < 5:
         return ["SHEL.L", "AZN.L", "HSBA.L", "ULVR.L", "BP.L"]
...
def update_uk_stocks():
    tickers = get_uk_tickers()
    stock_data = []
    count = 0

    print("Starting UK data extraction...")
    ...
    with open('uk_data.json', 'w') as f:
        json.dump(stock_data, f, indent=4, cls=NumpyEncoder)
        
    print(f"\n--- SUCCESS ---")
    print(f"Data saved: {len(stock_data)} valid UK stocks processed successfully into uk_data.json.")
