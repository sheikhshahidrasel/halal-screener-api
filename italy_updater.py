...
def get_italy_tickers():
    ...
    print("Fetching official Italy stock list automatically from TradingView Live Scanner...")
    
    # TradingView Live Scanner API Endpoint for Italy
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for item in data.get("data", []):
                ticker = item["d"][0]
                # Yahoo Finance uses .MI for Italian Stocks (Borsa Italiana)
                if not ticker.endswith('.MI'):
                     italy_tickers.add(f"{ticker}.MI")
                else:
                     italy_tickers.add(ticker)
            print(f"Successfully loaded {len(italy_tickers)} Italy tickers from TradingView API.")
        else:
            print(f"TradingView API returned status code {response.status_code}")
    except Exception as e:
        print(f"Error fetching Italy tickers from TradingView: {e}")

    final_tickers = list(italy_tickers)
    
    # Fallback in case API fails
    if len(final_tickers) < 5:
         return ["ENEL.MI", "ENI.MI", "ISP.MI", "UCG.MI", "RACE.MI"]
...
def update_italy_stocks():
    tickers = get_italy_tickers()
    stock_data = []
    count = 0

    print("Starting Italy data extraction...")
    ...
    # Saved entirely as a Italy specific JSON
    with open('italy_data.json', 'w') as f:
        json.dump(stock_data, f, indent=4, cls=NumpyEncoder)
        
    print(f"\n--- SUCCESS ---")
    print(f"Data saved: {len(stock_data)} valid Italy stocks processed successfully into italy_data.json.")
