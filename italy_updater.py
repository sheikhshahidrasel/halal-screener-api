...
def get_italy_tickers():
    ...
    # Fetch Major Italy Indices via Wikipedia Automatically (Borsa Italiana)
    wiki_sources = [
        ("https://en.wikipedia.org/wiki/FTSE_MIB", "MI")
    ]
    
    print("Fetching official Italy (Borsa Italiana) stock list automatically...")
    for url, suffix in wiki_sources:
        fetched = fetch_wiki_tickers(session, url, suffix)
        italy_tickers.update(fetched)
        print(f"Loaded {len(fetched)} tickers from index source.")
        time.sleep(2)

    final_tickers = list(italy_tickers)
    print(f"Successfully compiled a total of {len(final_tickers)} unique Italy tickers.")
